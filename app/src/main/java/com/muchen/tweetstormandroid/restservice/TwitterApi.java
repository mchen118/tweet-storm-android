...
public class TwitterApi {
    ...
    private int backUpToLastNonLetterCodePoint(String string, int index){
        char ch = string.charAt(index);
        if (Character.isHighSurrogate(ch)) {
            index--;
        } else if (Character.isLetter(ch) && Character.isLetter(string.charAt(index + 1))) {
            index--;
            while (Character.isLetter(string.charAt(index))) {
                index--;
            }
        }
        return index;
    ...
    public void tweetDraft(Draft draft) throws IOException, RuntimeException, TwitterApiException {
        ...
        while (beginIndex < bodyTweet.length()) {
            endIndex = beginIndex + Constants.TWEET_MAX_CHAR - headerLength;
            if (endIndex >= bodyTweet.length()) {
                endIndex = bodyTweet.length() - 1;
            } else {
                endIndex = backUpToLastNonLetterCodePoint(bodyTweet, endIndex);
            }

            TwitterTextParseResults parseResult = TwitterTextParser.parseTweet(
                        bodyTweet.substring(beginIndex, endIndex + 1));
